...
async function getDataFromSisi(userData) {
	let allData = userData.map(async (item) => {
...
async function combinator(urls) {
	...
	for (const url of urls) {
		try {
			const response = await axios.post(`https://stud-api.num.edu.mn/topMenus/TopSchedules?courseid=${url}`);
...
calc.post("/", async (req, res) => {
	...
	const promisedData = await getDataFromSisi(userData);
